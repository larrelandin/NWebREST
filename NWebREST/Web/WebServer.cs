...
using Zooking.MF.Helpers.Extensions;
using Zooking.MF.Helpers.ThreadUtil;
...
namespace NWebREST.Web
{
	...
	internal class WebServer
	{
		...
		private EndPoint InterpretRequest(string rawData)
		{
		    ...
		    if (rawData.Length > 5)
		    {
		        if (rawData.Substring(0, 5).Contains("POST"))
		        {
		            post = true;
		            commandData = rawData.Substring(6, rawData.Length - 6);
                    Debug.Print("NWebREST.InterpretRequest - POST request");
		        }
                else if (rawData.Substring(0, 5).Contains("GET"))
                {
                    commandData = rawData.Substring(5, rawData.Length - 5);
                    Debug.Print("NWebREST.InterpretRequest - GET request");
                }
                else
                {
                    return null;
            ...
            // Add arguments from post
		    if (post)
		    {
		        var tempArray = new string[parts.Length + 2];
		...
		private void StartServer()
		{
			using (server = new Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp))
			{
				...
				while (!_cancel)
				{
					//Thread acceptThread = new Thread(DoWebServerAccept);
					//acceptThread.Start();
					//if(!acceptThread.Join(30000))
					//{
					//	acceptThread.Abort();
					//}
					DoWebServerAccept();
		...
		private void DoWebServerAccept()
		{
